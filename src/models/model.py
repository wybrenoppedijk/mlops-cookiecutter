...
class MyAwesomeModel(LightningModule):
    def __init__(self, config):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(
                ...
                out_channels=hparams["max_channel_size"] // 2,
    ...
    def forward(self, x):
        ...
        if x.shape[1] != self.config.model["img_channels"]:
            raise ValueError(
                "Expected input with {} channels, got {}".format(
                    self.config.model["img_channels"], x.shape[1]
                )
            )
        x = self.conv(x)
        x = x.view(x.size(0), -1)
    ...
    def validation_step(self, *args, **kwargs) -> STEP_OUTPUT:
        ...
        if self.current_epoch % 2 == 0:
            ...
            self.logger.experiment.log(
                {"Intermediate Representation": wandb.Image(plt2)}
            )
